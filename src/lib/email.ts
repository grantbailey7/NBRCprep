...
export async function sendPasswordResetEmail(email: string, resetUrl: string, name?: string | null) {
  await resend.emails.send({
    from: 'NBRCprep <no-reply@nbrcprep.app>',
    to: email,
    subject: 'Reset your NBRCprep password',
    html: `
<!DOCTYPE html>
<html>
...
export async function sendWelcomeEmail(email: string, name?: string | null) {
  ...
  await resend.emails.send({
    from: 'Grant from NBRCprep <grant@nbrcprep.app>',
    to: email,
...
export async function sendDripEmail2(email: string, name?: string | null) {
  ...
  await resend.emails.send({
    from: 'Grant from NBRCprep <grant@nbrcprep.app>',
    to: email,
    subject: 'The study mistake that fails most RT candidates',
...
export async function sendDripEmail3(email: string, name?: string | null) {
  ...
  await resend.emails.send({
    from: 'Grant from NBRCprep <grant@nbrcprep.app>',
    to: email,
    subject: 'What every RT who passes the NBRC does differently',
...
export async function sendDripEmail4(email: string, name?: string | null) {
  ...
  await resend.emails.send({
    from: 'Grant from NBRCprep <grant@nbrcprep.app>',
    to: email,
    subject: 'Why 400 targeted flashcards beats a stack of textbooks',
...
export async function sendDripEmail5(email: string, name?: string | null) {
  ...
  const body = `
    ...
    ${h2('On the topic of readiness')}
    ${p("The closest thing to actual exam-day preparation is taking a full-length timed simulation. Nothing else tells you whether your pacing is right or shows you how it feels to sustain focus through a full-length NBRC exam.")}

    ${btn('Unlock full exam simulations →', 'https://nbrcprep.app/pricing')}
  ...
  await resend.emails.send({
    from: 'Grant from NBRCprep <grant@nbrcprep.app>',
    to: email,
    subject: 'Two weeks since you signed up — honest question',
    html: wrap('Have you scheduled your exam yet?', body, email),
  })
}
...
export async function sendDripEmail6(email: string, name?: string | null) {
  ...
  await resend.emails.send({
    from: 'Grant from NBRCprep <grant@nbrcprep.app>',
    to: email,
    subject: "Last thing I'll say about upgrading (I promise)",
